...
const benefits = [
  ...
  },
  {
    ...
    icon: (
      <svg className="w-7 h-7" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V19.5a2.25 2.25 0 0 0 2.25 2.25h.75m0-11.25h.008v.008H12V9.75Zm.375 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z" />
      </svg>
    ),
  },
  {
    ...
    <section className="relative py-16 md:py-24">
      ...
      <div className="relative container-narrow">
        <div className="text-center mb-12">
          <p className="text-brand-gold uppercase tracking-widest text-sm font-bold">WHY US?</p>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white mt-2">
            Sell To Adam Has Your Back
          </h2>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {benefits.map((b) => (
            <div key={b.title} className="flex gap-4">
              <div className="text-brand-gold flex-shrink-0 mt-1">{b.icon}</div>
              <div>
                <h3 className="font-bold text-lg text-white">{b.title}</h3>
                <p className="text-white/80 mt-1 text-sm leading-relaxed">{b.desc}</p>
              </div>
            </div>
          ))}
        </div>
